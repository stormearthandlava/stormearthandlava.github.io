...
This means that we can actually open the fight with a precasted {{ site.data.spell.lvb }}, followed by {{ site.data.spell.fs }} on pull, to still ensure a 100% crit of {{ site.data.spell.lvb }}. That's nice quality of life, but it doesn't change anything, but our ability to properly switch targets one cast earlier than before.
...
{{ site.data.talent.ee }} replaces Exposed Elements in the level 15 row. The new talent {{ site.data.talent.sop }} joins in the otherwise empty left level 90 talent slot.
...
- Mechanic: empowered {{ site.data.spell.frs }} generate 8 Maelstrom
