...
- You never want to cast {{ site.data.spell.lvb }} except before {{ site.data.talent.eb }} (which means you don't want to press it for multiple seconds even tho {{ site.data.talent.lvs }} is active),
...
It's also worth to note that it's very easily to consume {{ site.data.talent.sop }} with another spell, losing a lot of potential damage in the process.

Sounds easy right? It's not that hard once the get the idea going and it's clear in your head. But it's also **very easy** to mess it up or to struggle to pick up the rotation. Trying to make everything work all at once without breaking it down into simple steps is a hassle, and even when mastered, it's common to make silly mistakes.
It's also very counterintuitive to sit on {{ site.data.talent.lvs }} for 7-8 seconds which happens fairly often.

By the way, I didn't talk about {{ site.data.talent.lightning_rod }} at all since it's mostly a passive gain always present during important damage events since {{ site.data.talent.eb }} applies it to your target. But it's also a significant part of our damage. Omitting it made everything more simple. 

<hr>

### Chain Lightning target scaling

I'm not going to detail this section too much because Bloodmallet already did an excellent analysis [in this article](https://stormearthandlava.com/blog/2023/02/06/chain-lightning-woes.html). This post is 8 months old and still accuratly explains why Quadratic scaling is both nice and bad for {{ site.data.spell.cl }}.
